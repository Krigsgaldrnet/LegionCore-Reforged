...
void ObjectMgr::GetLocaleString(std::vector<std::string> const& data, LocaleConstant localeConstant, std::string& value)
{
...
void ObjectMgr::LoadWorldVisibleDistance()
{
    ...
    do
    {
        ...
        {
            TC_LOG_INFO("server.loading", ">> Loaded world visible distance type %u error", type);
            continue;
        }
    ...
    TC_LOG_INFO("server.loading", ">> Loaded %u world visible distance in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadWorldMapDiffycultyStat()
{
    ...
    do
    {
        ...
        {
            TC_LOG_INFO("server.loading", ">> Loaded world map difficulty stats error - mapId: %i, difficultyId: %u", mapId, difficultyId);
            continue;
        }
    ...
    TC_LOG_INFO("server.loading", ">> Loaded %u world map difficulty stats in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadWorldRateInfo()
{
    ...
    do
    {
        ...
        {
            TC_LOG_INFO("server.loading", ">> Loaded world rate type %u error", type);
            continue;
        }
    ...
    TC_LOG_INFO("server.loading", ">> Loaded %u world rate in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadCreatureLocales()
{
    ...
    TC_LOG_INFO("server.loading", ">> Loaded %u creature locale strings in %u ms", static_cast<uint32>(_creatureLocaleStore.size()), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadWDBCreatureTemplates()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server.loading", ">> Loaded 0 creature template definitions. DB table `creature_template_wdb` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("server.loading", ">> Loaded %u creature WDB templates in %u ms", static_cast<uint32>(_creatureTemplateStoreMap.size()), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::CheckCreatureTemplateWDB(CreatureTemplate* cInfo)
{
    ...
    for (uint8 i = 0; i < MAX_CREATURE_MODELS; ++i)
    {
        if (cInfo->Modelid[i])
        {
            ...
            if (!displayEntry)
            {
                TC_LOG_ERROR("sql.sql", "Creature (Entry: %u) lists non-existing Modelid[0] id (%i), this can crash the client.", cInfo->Entry, cInfo->Modelid[i]);
                cInfo->Modelid[i] = 0;
            }
            ...
            {
                TC_LOG_ERROR("sql.sql", "No model data exist for `Modelid[%u]` = %i listed by creature (Entry: %u).", i, cInfo->Modelid[i], cInfo->Entry);
    ...
    if (!displayScaleEntry)
        TC_LOG_ERROR("sql.sql", "Creature (Entry: %u) does not have any existing display id in Modelid1/Modelid2/Modelid3/Modelid[3].", cInfo->Entry);
...
void ObjectMgr::LoadCreatureTemplates()
{
    ...
    do
    {
        ...
        if (!fields[index].IsNull())
            creatureTemplate.Movement.InteractionPauseTimer = fields[index].GetUInt32();
        index++;
...
void ObjectMgr::LoadCreatureDifficultyStat()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server.loading", ">> Loaded 0 creature difficulty stat definitions. DB table `creature_difficulty_stat` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("server.loading", ">> Loaded %u creature difficulty stat  definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::CheckCreatureTemplate(CreatureTemplate const* cInfo)
{
    ...
    if (!entry)
    {
        TC_LOG_ERROR("sql.sql", "Creature (template id: %u) has invalid faction (faction template id) #%u", cInfo->Entry, cInfo->faction);
...
CreatureModelInfo const* ObjectMgr::GetCreatureModelRandomGender(uint32* displayID)
{
    ...
    if (modelInfo->displayId_other_gender != 0 && urand(0, 1) == 0)
    {
        ...
        if (!minfo_tmp)
            TC_LOG_ERROR("sql.sql", "Model (Entry: %u) has displayId_other_gender %u not found in table `creature_model_info`. ", *displayID, modelInfo->displayId_other_gender);
        else
        {
            // DisplayID changed
...
void ObjectMgr::LoadCreatureModelInfo()
{
    ...
    do
    {
        ...
        if (!creatureDisplay)
        {
            TC_LOG_ERROR("sql.sql", "Table `creature_model_info` has model for not existed display id (%u).", displayId);
        ...
        {
            TC_LOG_ERROR("sql.sql", "Table `creature_model_info` has not existed alt.gender model (%u) for existed display id (%u).", modelInfo.displayId_other_gender, displayId);
        ...
        {
            TC_LOG_ERROR("sql.sql", "Table `creature_model_info` has not existed alt.hostileId model (%u) for existed display id (%u).", modelInfo.hostileId, displayId);
            modelInfo.hostileId = 0;
        }
    ...
    do
    {
        ...
        switch (linkType)
        {
            case CREATURE_TO_CREATURE:
            {
                ...
                if (!slave)
                {
                    TC_LOG_ERROR("sql.sql", "Couldn't get creature data for GUIDLow " UI64FMTD "", guidLow);
                    error = true;
                    break;
                }
                ...
                if (!master)
                {
                    TC_LOG_ERROR("sql.sql", "Couldn't get creature data for GUIDLow " UI64FMTD "", linkedGuidLow);
                    error = true;
                    break;
                }
                ...
                {
                    TC_LOG_ERROR("sql.sql", "Creature '" UI64FMTD "' linking to '%lu' on an unpermitted map.", guidLow, linkedGuidLow);
                    error = true;
                    break;
                }
                ...
                {
                    TC_LOG_ERROR("sql.sql", "LinkedRespawn: Creature '" UI64FMTD "' linking to '%lu' with not corresponding spawnMask", guidLow, linkedGuidLow);
                    error = true;
                    break;
                }
            ...
            case CREATURE_TO_GO:
            {
                ...
                if (!master)
                {
                    TC_LOG_ERROR("sql.sql", "Couldn't get gameobject data for GUIDLow " UI64FMTD "", linkedGuidLow);
                    error = true;
                    break;
                }
            ...
            case GO_TO_GO:
            {
                ...
                if (!slave)
                {
                    TC_LOG_ERROR("sql.sql", "Couldn't get gameobject data for GUIDLow " UI64FMTD "", guidLow);
                    error = true;
                    break;
                }
...
void ObjectMgr::LoadCreatures()
{
    ...
    do
    {
        ...
        // {
            ...
            // {
                ...
                // {
                    ...
                    // TC_LOG_ERROR("sql.sql", "Table `creature` have clone npc %u witch stay too close (dist: %f). original npc guid %u. npc with guid %u will be deleted.", entry, distsq1, lastCreature->second->guid, guid);
                    // continue;
                // }
            // }else
        ...
        if (!mapEntry)
        {
            TC_LOG_ERROR("sql.sql", "Table `creature` have creature (GUID: " UI64FMTD ") that spawned at not existed map (Id: %u), skipped.", guid, data.mapid);
            continue;
        }
        ...
        if (!mapEntry->IsBattlegroundOrArena())
        {
            ...
            {
                TC_LOG_ERROR("sql.sql", "Table `creature` have creature (GUID: " UI64FMTD ") that have wrong spawn mask " UI64FMTD " including not supported difficulty modes for map (Id: %u) spawnMasks[data.mapid]: %lu.", guid, data.spawnMask, data.mapid, spawnMasks[data.mapid]);
        ...
        {
            TC_LOG_ERROR("sql.sql", "Table `creature` have creature (GUID: " UI64FMTD " Entry: %u) with `spawndist`< 0, set to 0.", guid, data.id);
            data.spawndist = 0.0f;
        }
        else if (data.movementType == RANDOM_MOTION_TYPE)
        {
            if (data.spawndist == 0.0f)
            {
                TC_LOG_ERROR("sql.sql", "Table `creature` have creature (GUID: " UI64FMTD " Entry: %u) with `MovementType`=1 (random movement) but with `spawndist`=0, replace by idle movement type (0).", guid, data.id);
        ...
        else if (data.movementType == IDLE_MOTION_TYPE)
        {
            if (data.spawndist != 0.0f)
            {
                TC_LOG_ERROR("sql.sql", "Table `creature` have creature (GUID: " UI64FMTD " Entry: %u) with `MovementType`=0 (idle) have `spawndist`<>0, set to 0.", guid, data.id);
                data.spawndist = 0.0f;
            }
        }
        ...
        if (data.phaseMask == 0)
        {
            TC_LOG_ERROR("sql.sql", "Table `creature` have creature (GUID: " UI64FMTD " Entry: %u) with `phaseMask`=0 (not visible for anyone), set to 1.", guid, data.id);
            data.phaseMask = 1;
        }
...
void ObjectMgr::LoadCreatureAIInstance()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server.loading", ">> Loaded 0 creature AI instance. DB table `creature_ai_instance` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("server.loading", ">> Loaded %u creature AI instance in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadPersonalLootTemplate()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server.loading", ">> Loaded 0 personal loot. DB table `personal_loot_template` is empty.");
        return;
    }
    ...
    TC_LOG_INFO("server.loading", ">> Loaded %u personal loot in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
bool ObjectMgr::MoveCreData(ObjectGuid::LowType const& guid, uint32 mapId, Position pos)
{
    ...
    if (Map* map = sMapMgr->CreateBaseMap(mapId))
    {
        ...
        if (!map->Instanceable() && map->IsGridLoaded(data.posX, data.posY))
        {
            ...
            {
                TC_LOG_ERROR("misc", "AddCreature: cannot add creature entry %lu to map", guid);
                delete creature;
                return false;
            }
        }
...
void ObjectMgr::LoadGameobjects()
{
    ...
    do
    {
        ...
        if (!IsTransportMap(data.mapid) && data.spawnMask & ~spawnMasks[data.mapid])
        {
            TC_LOG_ERROR("sql.sql", "Table `gameobject` has gameobject (GUID: " UI64FMTD " Entry: %u) that has wrong spawn mask " UI64FMTD " including not supported difficulty modes for map (Id: %u), skip", guid, data.id, data.spawnMask, data.mapid);
            data.spawnMask = spawnMasks[data.mapid];
        ...
        {
            TC_LOG_ERROR("sql.sql", "Table `gameobject` has gameobject (GUID: " UI64FMTD " Entry: %u) with invalid rotation2 (%f) value, skip", guid, data.id, data.rotation.x);
            continue;
        }

        if (data.rotation.y < -1.0f || data.rotation.y > 1.0f)
        {
            TC_LOG_ERROR("sql.sql", "Table `gameobject` has gameobject (GUID: " UI64FMTD " Entry: %u) with invalid rotation3 (%f) value, skip", guid, data.id, data.rotation.y);
            continue;
        }
        ...
        {
            TC_LOG_ERROR("sql.sql", "Gameobject (GUID: " UI64FMTD " Entry %u GoType: %u) GAMEOBJECT_TYPE_MAP_OBJ_TRANSPORT not spawn in world", guid, entry, gInfo->type);
            continue;
        }
        ...
        if (gInfo->type == GAMEOBJECT_TYPE_GARRISON_SHIPMENT)
        {
            ...
            {
                TC_LOG_ERROR("sql.sql", "Gameobject (GUID: " UI64FMTD " Entry %u GoType: %u) GAMEOBJECT_TYPE_GARRISON_SHIPMENT no shipment conteiner ID", guid, entry, gInfo->type);
                continue;
            }
    ...
    TC_LOG_INFO("server.loading", ">> Loaded %lu gameobjects in %u ms", static_cast<unsigned long>(_gameObjectDataStore.size()), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadItemTemplates()
{
    ...
    TC_LOG_INFO("server.loading", ">> Loaded %u item templates from ItemSparse.db2 in %u ms", sparseCount, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadVehicleTemplateAccessories()
{
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR("sql.sql", "Table `vehicle_template_accessory`: creature template entry %u does not exist.", uiEntryOrAura);
            continue;
        }

        if (uiEntryOrAura < 0 && !sSpellMgr->GetSpellInfo(uiEntryOrAura * -1))
        {
            TC_LOG_ERROR("sql.sql", "Table `vehicle_template_accessory`: aura %u does not exist.", uiEntryOrAura);
            continue;
        }
        ...
        {
            TC_LOG_ERROR("sql.sql", "Table `vehicle_template_accessory`: creature template entry %u has no data in npc_spellclick_spells", uiEntryOrAura);
            continue;
        }
    ...
    if (!result)
    {
        TC_LOG_ERROR("server.loading", ">> Loaded 0 vehicle attachment offset. DB table `vehicle_attachment_offset` is empty.");
        return;
    }
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR("sql.sql", "Table `vehicle_attachment_offset`: creature template entry %u does not exist.", entry);
            continue;
        }
...
void ObjectMgr::LoadPetStats()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server.loading", ">> Loaded 0 pet stats. DB table `pet_stats` is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (!sObjectMgr->GetCreatureTemplate(entry))
        {
            TC_LOG_ERROR("sql.sql", "Wrong creature id %u in `pet_stats` table, ignoring.", entry);
            continue;
        }
    ...
    TC_LOG_INFO("server.loading", ">> Loaded %u pet stats definitions in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadPlayerInfo()
{
    ...
    TC_LOG_INFO("server.loading", "Loading Player Create Skill Data...");
    {
        ...
        TC_LOG_INFO("server.loading", ">> Loaded player create skills in %u s", GetMSTimeDiffToNow(oldMSTime) / IN_MILLISECONDS);
    }
    ...
    TC_LOG_INFO("server.loading", "Loading Player Create Level Stats Data...");
    {
        ...
        for (uint8 race = 0; race < MAX_RACES; ++race)
        {
            ...
            for (uint8 class_ = 0; class_ < MAX_CLASSES; ++class_)
            {
                ...
                {
                    TC_LOG_ERROR("sql.sql", "Race %i Class %i does not have stats data for base lvl (%i)!", race, class_, level - 1);
                    exit(1);
                }
    ...
    TC_LOG_INFO("server.loading", "Loading Player Create XP Data...");
    {
        ...
        for (uint8 level = 1; level < sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL); ++level)
        {
            ...
            {
                TC_LOG_INFO("misc", "Level %i does not have XP for level data. Using data of level [%i] + 12000.", level + 1, level);
    ...
    TC_LOG_INFO("server.loading", "Loading Player Create Quests Data...");
    {
        oldMSTime = getMSTime();
        //                                                0     1      2       3
        QueryResult result = WorldDatabase.Query("SELECT race, class, questid FROM playercreateinfo_quest");
        if (!result)
            TC_LOG_INFO("server.loading", ">> Loaded 0 custom player create quests. DB table `playercreateinfo_quest` is empty.");
        else
        {
            uint32 count = 0;
            do
            {
                ...
                if (current_race >= MAX_RACES)
                {
                    TC_LOG_ERROR("sql.sql", "Wrong race %u in `playercreateinfo_quest` table, ignoring.", current_race);
                    continue;
                }
                ...
                if (current_class >= MAX_CLASSES)
                {
                    TC_LOG_ERROR("sql.sql", "Wrong class %u in `playercreateinfo_quest` table, ignoring.", current_class);
                    continue;
                }
                ...
                {
                    TC_LOG_ERROR("sql.sql", "Quest id %u (race %u class %u) in `playercreateinfo_quest` table but not listed in `quest_template`, ignoring.", quest_id, current_race, current_class);
                    continue;
                }
            ...
            TC_LOG_INFO("server.loading", ">> Loaded %u custom player create quests in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
        }
    }
}
...
void ObjectMgr::LoadPageTexts()
{
    ...
    for (auto const& itr : _pageTextStore)
        if (itr.second.NextPageID)
            ...
            {
                TC_LOG_ERROR("sql.sql", "Page text (Id: %u) has not existing next page (Id: %u)", itr.first, itr.second.NextPageID);
            }
...
void ObjectMgr::LoadPageTextLocales()
{
    ...
    TC_LOG_INFO("server.loading", ">> Loaded %lu PageText locale strings in %u ms", static_cast<unsigned long>(_pageTextLocaleStore.size()), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadInstanceEncounters()
{
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR("sql.sql", "Table `instance_encounters` has an encounter %u (%s) marked as final for invalid dungeon id %u, skipped!", entry, dungeonEncounter->Name->Str[LOCALE_enUS], lastEncounterDungeon);
            continue;
        }
        ...
        if (lastEncounterDungeon)
        {
            ...
            {
                TC_LOG_ERROR("sql.sql", "Table `instance_encounters` specified encounter %u (%s) as last encounter but %u (%s) is already marked as one, skipped!", entry, dungeonEncounter->Name->Str[LOCALE_enUS], itr->second->ID, itr->second->Name->Str[LOCALE_enUS]);
                continue;
            }
        ...
        switch (creditType)
        {
            case ENCOUNTER_CREDIT_KILL_CREATURE:
            {
                ...
                if (!creatureInfo)
                {
                    TC_LOG_ERROR("sql.sql", "Table `instance_encounters` has an invalid creature (entry %u) linked to the encounter %u (%s), skipped!", creditEntry, entry, dungeonEncounter->Name->Str[LOCALE_enUS]);
                    continue;
                }
            ...
            case ENCOUNTER_CREDIT_CAST_SPELL:
                ...
                {
                    TC_LOG_ERROR("sql.sql", "Table `instance_encounters` has an invalid spell (entry %u) linked to the encounter %u (%s), skipped!", creditEntry, entry, dungeonEncounter->Name->Str[LOCALE_enUS]);
                    continue;
                }
                break;
            default:
                TC_LOG_ERROR("sql.sql", "Table `instance_encounters` has an invalid credit type (%u) for encounter %u (%s), skipped!", creditType, entry, dungeonEncounter->Name->Str[LOCALE_enUS]);
                continue;
        }

        if (dungeonEncounter->DifficultyID > 0 && dungeonEncounter->DifficultyID != DungeonDifficult)
        {
            TC_LOG_ERROR("sql.sql", "Table `instance_encounters` overvrite dificult from dbc for encounter %u. DBC %i, DB %i", entry, dungeonEncounter->DifficultyID, DungeonDifficult);
...
void ObjectMgr::LoadAccessRequirements()
{
    ...
    do
    {
        ...
        if (ar.achievement_A)
        {
            ...
            {
                TC_LOG_ERROR("sql.sql", "Required achievement_A %u not exist for map %u difficulty %u, remove quest done requirement.", ar.achievement_A, mapid, difficulty);
                ar.achievement_A = 0;
            }
        }
...
void ObjectMgr::LoadGameObjectLocales()
{
    ...
    TC_LOG_INFO("server.loading", ">> Loaded %lu gameobject locale strings in %u ms", static_cast<unsigned long>(_gameObjectLocaleStore.size()), GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadGameObjectQuestVisual()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server.loading", ">> Loaded 0 gameobject quest visual. DB table `gameobject_quest_visual` is empty.");
        return;
    }
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR("sql.sql", "Gameobject (Entry: %u) not exist on table `gameobject_quest_visual`", goID);
            continue;
        }
        if (questID && !sQuestDataStore->GetQuestTemplate(questID))
        {
            TC_LOG_ERROR("sql.sql", "Quest (ID: %u) not exist on table `gameobject_quest_visual`", questID);
            continue;
        }
    ...
    TC_LOG_INFO("server.loading", ">> Loaded %u game object quest visual in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
}
...
void ObjectMgr::LoadCurrencysLoot()
{
    ...
    do
    {
        ...
        if (type < 1)
        {
            TC_LOG_INFO("server.loading", "Currency 'type' can not be < 1 (entry = %u type = %i)", entry, type);
            continue;
        }
        if (type > 3)
        {
            TC_LOG_INFO("server.loading", "Currency 'type' can not be > 3 (entry = %u type = %i)", entry, type);
            continue;
        }
    ...
    if (count)
        TC_LOG_INFO("server.loading", "Loaded %u currency loot definition", count);
    else
        TC_LOG_INFO("server.loading", "Loaded 0 currency loot definition. Table is empty!");
}
    ...
    do
    {
        ...
        if (repOnKill.RepFaction)
        {
            ...
            if (!factionEntry1)
            {
                ...
                TC_LOG_ERROR("sql.sql", "Faction (faction.dbc) %u does not exist but is used in `creature_onkill_reputation`", repOnKill.RepFaction);
                continue;
            }
        }
...
void ObjectMgr::LoadCreatureClassLevelStats()
{
    ...
    for (auto &v : *ctc)
        for (uint16 lvl = v.second.minlevel; lvl <= v.second.maxlevel; ++lvl)
            ...
            {
                TC_LOG_ERROR("sql.sql", "Missing base stats for creature class %u level %u", v.second.unit_class, lvl);
            }
...
void ObjectMgr::LoadPhaseDefinitions()
{
    ...
    do
    {
        ...
        if (pd.terrainswapmap > 0)
        {
            ...
            if (!map)
            {
                TC_LOG_ERROR("sql.sql", "DB table `phase_definitions` has not existen terrainswapmap %u", pd.terrainswapmap);
                continue;
            }
        }
...
void ObjectMgr::RestructCreatureGUID()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server.loading", "RestructCreatureGUID failed");
        return;
    }
    ...
    TC_LOG_INFO("server.loading", "RestructCreatureGUID: Last guid %u", newGUID);
}
...
void ObjectMgr::RestructGameObjectGUID()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server.loading", "RestructGameObjectGUID failed");
        return;
    }
    ...
    TC_LOG_INFO("server.loading", "RestructGameObjectGUID: Last guid %u", newGUID);
}
...
void ObjectMgr::LoadResearchSiteToZoneData()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server.loading", ">> Loaded 0 archaeology zones. DB table `archaeology_zones` is empty.");
        return;
    }
    ...
    do
    {
        ...
        {
            TC_LOG_ERROR("sql.sql", "DB table `archaeology_zones` has data for nonexistant site id %u", site_id);
            continue;
        }
    ...
    /*for (auto const& itr : sDB2Manager._researchSiteDataMap)
    {
        if (itr.second.zone == 0 || itr.second.level == 0xFF || itr.second.branch_id == 0)
            TC_LOG_ERROR("sql.sql", "DB table `archaeology_zones` has not full or does not have data for site id %u: "
            "zone %u level %u branch_id %u",
            itr.second.entry->ID, itr.second.zone, itr.second.level, itr.second.branch_id);
    }*/

    TC_LOG_INFO("server.loading", ">> Loaded %u archaeology zones.", counter);
}
...
void ObjectMgr::LoadDigSitePositions()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server.loading", ">> Loaded 0 dig site positions. DB table `archaeology_digsites` is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (!x && !y)
        {
            ...
            TC_LOG_ERROR("sql.sql", "DB table `archaeology_digsites` has data for point idx:%u x:%f y:%f at map %u ADD!", idx, x, y, map);
        }
        ...
        if (!added)
        {
            TC_LOG_ERROR("sql.sql", "DB table `archaeology_digsites` has data for point idx:%u x:%f y:%f at map %u that does not belong to any digsite!", idx, x, y, map);
            continue;
        }
    ...
    for (auto& itr : sDB2Manager._researchSiteDataMap)
    {
        ...
        {
            //TC_LOG_ERROR("sql.sql", "Archaeology research site %u has less that %u dig site positions! (%u)", data.entry->ID, MAX_DIGSITE_FINDS, siteCount);
            if (!siteCount)
    ...
    TC_LOG_INFO("server.loading", ">> Loaded %u dig site positions _researchSiteDataMap %zu", counter, sDB2Manager._researchSiteDataMap.size());
}
...
void ObjectMgr::LoadScenarioData()
{
    ...
    if (result)
    {
        ...
        TC_LOG_INFO("server.loading", ">> Loaded %u Scenario data.", counter);
    }
    else
        TC_LOG_INFO("server.loading", ">> Loaded 0 Scenario data. DB table `scenario_data` is empty.");
}
...
void ObjectMgr::LoadScenarioSpellData()
{
    ...
    if (result)
    {
        ...
        TC_LOG_INFO("server.loading", ">> Loaded %u Scenario Step Spell Data.", counter);
    }
    else
        TC_LOG_INFO("server.loading", ">> Loaded 0 Scenario Step Spell data. DB table `scenario_step_spells` is empty.");
}
...
void ObjectMgr::LoadCreatureActionData()
{
    ...
    if (result)
    {
        ...
        TC_LOG_INFO("server.loading", ">> Loaded %u creature_action data.", counter);
    }
    else
        TC_LOG_INFO("server.loading", ">> Loaded 0 creature_action data. DB table `creature_action` is empty.");
}
...
void ObjectMgr::LoadSeamlessTeleportData()
{
    ...
    if (result)
    {
        ...
        TC_LOG_INFO("server.loading", ">> Loaded %u seamless teleport data.", counter);
    }
    else
        TC_LOG_INFO("server.loading", ">> Loaded 0 seamless teleport data. DB table `world_seamless_teleport` is empty.");
}
...
void ObjectMgr::LoadDisplayChoiceData()
{
    ...
    if (auto rewards = WorldDatabase.Query("SELECT ChoiceId, ResponseId, TitleId, PackageId, SkillLineId, SkillPointCount, ArenaPointCount, HonorPointCount, Money, Xp, SpellID FROM playerchoice_response_reward"))
    {
        do
        {
            ...
            {
                TC_LOG_ERROR("sql.sql", "Table `playerchoice_response_reward` references non-existing Title %d for ChoiceId %d, ResponseId: %d, set to 0", reward->TitleId, choiceId, responseId);
                reward->TitleId = 0;
            }

            if (reward->PackageId && !sDB2Manager.GetQuestPackageItems(reward->PackageId))
            {
                TC_LOG_ERROR("sql.sql", "Table `playerchoice_response_reward` references non-existing QuestPackage %d for ChoiceId %d, ResponseId: %d, set to 0", reward->TitleId, choiceId, responseId);
                reward->PackageId = 0;
            }

            if (reward->SkillLineId && !sSkillLineStore.LookupEntry(reward->SkillLineId))
            {
                TC_LOG_ERROR("sql.sql", "Table `playerchoice_response_reward` references non-existing SkillLine %d for ChoiceId %d, ResponseId: %d, set to 0", reward->TitleId, choiceId, responseId);
    ...
    if (auto rewards = WorldDatabase.Query("SELECT ChoiceId, ResponseId, ItemId, BonusListIDs, Quantity FROM playerchoice_response_reward_item ORDER BY `Index` ASC"))
    {
        do
        {
            ...
            {
                TC_LOG_ERROR("sql.sql", "Table `playerchoice_response_reward_item` references non-existing player choice reward for ChoiceId %d, ResponseId: %d, skipped", choiceId, responseId);
                continue;
            }

            if (!GetItemTemplate(itemId))
            {
                TC_LOG_ERROR("sql.sql", "Table `playerchoice_response_reward_item` references non-existing item %u for ChoiceId %d, ResponseId: %d, skipped", itemId, choiceId, responseId);
                continue;
            }
    ...
    if (auto rewards = WorldDatabase.Query("SELECT ChoiceId, ResponseId, CurrencyId, Quantity FROM playerchoice_response_reward_currency ORDER BY `Index` ASC"))
    {
        do
        {
            ...
            if (!responseItr->Reward)
            {
                TC_LOG_ERROR("sql.sql", "Table `playerchoice_response_reward_currency` references non-existing player choice reward for ChoiceId %d, ResponseId: %d, skipped", choiceId, responseId);
                continue;
            }

            if (!sCurrencyTypesStore.LookupEntry(currencyId))
            {
                TC_LOG_ERROR("sql.sql", "Table `playerchoice_response_reward_currency` references non-existing currency %u for ChoiceId %d, ResponseId: %d, skipped", currencyId, choiceId, responseId);
                continue;
            }
    ...
    if (auto rewards = WorldDatabase.Query("SELECT ChoiceId, ResponseId, FactionId, Quantity FROM playerchoice_response_reward_faction ORDER BY `Index` ASC"))
    {
        do
        {
            ...
            if (!responseItr->Reward)
            {
                TC_LOG_ERROR("sql.sql", "Table `playerchoice_response_reward_faction` references non-existing player choice reward for ChoiceId %d, ResponseId: %d, skipped", choiceId, responseId);
                continue;
            }

            if (!sFactionStore.LookupEntry(factionId))
            {
                TC_LOG_ERROR("sql.sql", "Table `playerchoice_response_reward_faction` references non-existing faction %u for ChoiceId %d, ResponseId: %d, skipped", factionId, choiceId, responseId);
                continue;
            }
...
void ObjectMgr::LoadPlayerChoicesLocale()
{
    ...
    if (auto result = WorldDatabase.Query("SELECT ChoiceID, ResponseID, locale, Header, Answer, Description, Confirmation FROM playerchoice_response_locale"))
    {
        ...
        do
        {
            ...
            {
                TC_LOG_ERROR("sql.sql", "Table `playerchoice_locale` references non-existing ChoiceId: %d for ResponseId %d locale %s, skipped", choiceId, responseId, localeName.c_str());
                continue;
            }
            ...
            {
                TC_LOG_ERROR("sql.sql", "Table `playerchoice_locale` references non-existing ResponseId: %d for ChoiceId %d locale %s, skipped", responseId, choiceId, localeName.c_str());
                continue;
            }
...
void ObjectMgr::LoadGameObjectActionData()
{
    ...
    if (result)
    {
        ...
        TC_LOG_INFO("server.loading", ">> Loaded %u gameobject_action data.", counter);
    }
    else
        TC_LOG_INFO("server.loading", ">> Loaded 0 gameobject_action data. DB table `gameobject_action` is empty.");
}
...
void ObjectMgr::LoadRaceAndClassExpansionRequirements()
{
    ...
    if (result)
    {
        ...
        TC_LOG_INFO("server.loading", ">> Loaded 0 race expansion requirements. DB table `race_unlock_requirement` is empty.");
...
void ObjectMgr::LoadCreatureOutfits()
{
    ...
    if (!result)
    {
        TC_LOG_INFO("server.loading", ">> Loaded 0 creature outfits. DB table `creature_template_outfits` is empty!");
        return;
    }
    ...
    do
    {
        ...
        if (!rEntry)
        {
            TC_LOG_INFO("server.loading", ">> Outfit entry %u in `creature_template_outfits` has incorrect race (%u).", entry, uint32(co.race));
            continue;
        }
        ...
        if (!cEntry)
        {
            TC_LOG_INFO("server.loading", ">> Outfit entry %u in `creature_template_outfits` has incorrect class (%u).", entry, uint32(co.Class));
            continue;
        }
        ...
        default:
            TC_LOG_INFO("server.loading", ">> Outfit entry %u in `creature_template_outfits` has invalid gender %u", entry, uint32(co.gender));
            continue;
        }
        ...
        for (uint32 j = 0; j < CreatureOutfit::max_outfit_displays; ++j)
        {
            ...
            if (displayInfo > 0) // entry
            {
                ...
                {
                    TC_LOG_INFO("server.loading", ">> Creature entry %u in `creature_template_outfits` has invalid (item entry, appearance) combination: %u, %u. Value in DB: %s", entry, item_entry, appearancemodid, std::to_string(displayInfo).c_str());
                    co.outfit[j] = 0;
                }
            }
            else // display
