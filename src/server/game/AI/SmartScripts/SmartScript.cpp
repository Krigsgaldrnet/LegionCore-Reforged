...
void SmartScript::ProcessAction(SmartScriptHolder& e, Unit* unit, uint32 var0, uint32 var1, bool bvar, const SpellInfo* spell, GameObject* gob)
{
    ...
    switch (e.GetActionType())
    {
        case SMART_ACTION_TALK:
        {
            ...
            TC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_TALK: talker: %s (GuidLow: %u), textGuid: %lu",
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsUnit(*itr))
                {
                    ...
                    if (e.action.castCustom.damageMax)
                        bp0 = urand(std::min(e.action.castCustom.damageMin, e.action.castCustom.damageMax), std::max(e.action.castCustom.damageMin, e.action.castCustom.damageMax));
                    ...
                    if (!(e.action.castCustom.flags & SMARTCAST_AURA_NOT_PRESENT) || !(*itr)->ToUnit()->HasAura(spellId))
                    {
                        ...
                        TC_LOG_DEBUG("scripts.ai", "Spell %u not casted because it has flag SMARTCAST_AURA_NOT_PRESENT and the target (Guid: %s Entry: %u Type: %u) already has the aura", spellId, (*itr)->GetGUID().ToString().c_str(), (*itr)->GetEntry(), uint32((*itr)->GetTypeId()));
                    TC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_CAST_CUSTOM:: Creature %u casts spell %u on target %u with castflags %u",
        ...
        case SMART_ACTION_BOSS_ANOUNCE:
        {
            ...
            TC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_BOSS_ANOUNCE: Creature %u Announce", me->GetGUIDLow());
            return;
        }
        case SMART_ACTION_BOSS_EVADE:
        {
            ...
            TC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_BOSS_EVADE: Creature %u EnterEvadeMode", me->GetGUIDLow());
            return;
        }
        ...
        case SMART_ACTION_SET_INST_DATA:
        {
            ...
            if (!instance)
            {
                TC_LOG_ERROR("sql.sql", "SmartScript: Event %u attempt to set instance data without instance script. EntryOrGuid %ld", e.GetEventType(), e.entryOrGuid);
                break;
            }
        ...
        {
            WorldObject* base = GetBaseObject();
            if (!base)
                break;
            ...
            if (targets)
            {
                ...
                for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
                {
                    ...
                    z += e.target.z;
                    o += e.target.o;
                    if (Creature* summon = base->SummonCreature(e.action.summonCreature.creature, x, y, z, o, static_cast<TempSummonType>(e.action.summonCreature.type), e.action.summonCreature.duration))
                    {
            ...
            if (e.GetTargetType() == SMART_TARGET_RANDOM_POSITION)
            {
                ...
                float objSize = base->GetObjectSize();
                float dist = objSize + (e.target.randomPos.range - objSize) * static_cast<float>(rand_norm());
                Position pos;
                if (e.target.randomPos.distance)
                {
                    Position center;
                    base->GetNearPoint2D(center, e.target.randomPos.distance, e.target.randomPos.angle * M_PI / 180.0f);
                    center.SimplePosXYRelocationByAngle(pos, dist, angle);
                }
                else
                    pos = base->GetFirstCollisionPosition(dist, angle);
                base->SummonCreature(e.action.summonCreature.creature, pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ(), angle, static_cast<TempSummonType>(e.action.summonCreature.type), e.action.summonCreature.duration);
            }
            ...
            if (e.GetTargetType() == SMART_TARGET_POSITION)
            {
                ...
                if (Creature* summon = base->SummonCreature(e.action.summonCreature.creature, x, y, z, e.target.o, static_cast<TempSummonType>(e.action.summonCreature.type), e.action.summonCreature.duration))
        ...
        case SMART_ACTION_SUMMON_GO:
        {
            ...
            if (e.GetTargetType() == SMART_TARGET_RANDOM_POSITION)
            {
                ...
                if (e.target.randomPos.distance)
                {
                    ...
                    pos = base->GetFirstCollisionPosition(dist, angle);
                base->SummonGameObject(e.action.summonGO.entry, pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ(), angle, 0, 0, 0, 0, e.action.summonGO.despawnTime);
            }
        ...
        case SMART_ACTION_CREATE_TIMED_EVENT:
        {
            ...
            ev.action = ac;
            InitTimer(ev);
            // a timed event may create another one while OnUpdate walks mStoredEvents: a push_back there
            // would reallocate the vector under the running reference
            mPendingStoredEvents.push_back(ev);
            break;
        }
        ...
        {
            ObjectList* casters = GetTargets(CreateEvent(SMART_EVENT_UPDATE_IC, 0, 0, 0, 0, 0, 0, SMART_ACTION_NONE, 0, 0, 0, 0, 0, 0, static_cast<SMARTAI_TARGETS>(e.action.cast.targetType), e.action.cast.targetParam1, e.action.cast.targetParam2, e.action.cast.targetParam3, 0, 0), unit);
            if (!casters)
                break;
                    ...
                    for (ObjectList::const_iterator it = targets->begin(); it != targets->end(); ++it)
                    {
                        if (IsUnit(*it))
                        {
                            if (!(e.action.cast.flags & SMARTCAST_AURA_NOT_PRESENT) || !(*it)->ToUnit()->HasAura(e.action.cast.spell))
                                ...
                                TC_LOG_DEBUG("scripts.ai", "Spell %u not casted because it has flag SMARTCAST_AURA_NOT_PRESENT and the target (Guid: %s Entry: %u Type: %u) already has the aura", e.action.cast.spell, (*it)->GetGUID().ToString().c_str(), (*it)->GetEntry(), uint32((*it)->GetTypeId()));
                        }
                    }
                }
            }
        ...
        {
            uint32 id = urand(std::min(e.action.randTimedActionList.entry1, e.action.randTimedActionList.entry2), std::max(e.action.randTimedActionList.entry1, e.action.randTimedActionList.entry2));
            if (e.GetTargetType() == SMART_TARGET_NONE)
            {
                TC_LOG_ERROR("sql.sql", "SmartScript: Entry %ld SourceType %u Event %u Action %u is using TARGET_NONE(0) for Script9 target. Please correct target_type in database.", e.entryOrGuid, e.GetScriptType(), e.GetEventType(), e.GetActionType());
                break;
        ...
        case SMART_ACTION_JUMP_TO_POS:
        {
            ...
            if (e.GetTargetType() == SMART_TARGET_ACTION_INVOKER && !unit)
                break;
        ...
        case SMART_ACTION_SEND_TARGET_TO_TARGET:
        {
            ...
            ObjectGuidVector const* storedTargets = GetStoredTargetGuids(e.action.sendTargetToTarget.id);
            if (!storedTargets)
            {
                delete targets;
                return;
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsCreature(*itr))
                {
                    if (SmartAI* ai = CAST_AI(SmartAI, (*itr)->ToCreature()->AI()))
                        ai->GetScript()->StoreTargetGuids(*storedTargets, e.action.sendTargetToTarget.id);
                    else
        ...
        case SMART_ACTION_SET_SCENATIO_ID:
        {
            ...
            TC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_SET_SCENATIO_ID: scenarioId %d", e.action.scenario.id);
        ...
        case SMART_ACTION_UPDATE_ACHIEVEMENT_CRITERIA:
        {
            ...
            TC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_UPDATE_ACHIEVEMENT_CRITERIA: type %d misc1 %d misc2 %d misc3 %d",
        ...
        case SMART_ACTION_SUMMON_CONVERSATION:
        {
            ...
            TC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_SUMMON_CONVERSATION: Conversation %d", e.action.conversation.id);
        ...
        case SMART_ACTION_SUMMON_ARIATRIGGER:
        {
            ...
            TC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_SUMMON_ARIATRIGGER: AreaTrigger %d", e.action.areatrigger.id);
        ...
        case SMART_ACTION_SUMMON_SCENE:
        {
            ...
            TC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_SUMMON_SCENE: scene %d duration %u", e.action.scene.id, e.action.scene.duration);
        ...
        case SMART_ACTION_START_TIMED_ACHIEVEMENT:
        {
            ...
            TC_LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_START_TIMED_ACHIEVEMENT: type %d misc1 %d misc2 %d misc3 %d",
                e.action.achievementCriteria.type, e.action.achievementCriteria.misc1, e.action.achievementCriteria.misc2, e.action.achievementCriteria.misc3);
        ...
        default:
            TC_LOG_ERROR("sql.sql", "SmartScript::ProcessAction: Entry %ld SourceType %u, Event %u, Unhandled Action type %u", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
            break;
    }
    ...
    if (e.link && e.link != e.event_id)
    {
        ...
        if (linked.GetActionType() && linked.GetEventType() == SMART_EVENT_LINK)
            ...
            TC_LOG_ERROR("sql.sql", "SmartScript::ProcessAction: Entry %ld SourceType %u, Event %u, Link Event %u not found or invalid, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.link);
    }
}
...
void SmartScript::InstallTemplate(SmartScriptHolder const& e)
{
    ...
    if (mTemplate)
    {
        TC_LOG_ERROR("sql.sql", "SmartScript::InstallTemplate: Entry %ld SourceType %u AI Template can not be set more then once, skipped.", e.entryOrGuid, e.GetScriptType());
        return;
    }
        ...
        case SMARTAI_TEMPLATE_CASTER:
            {
                ...
                AddEvent(SMART_EVENT_CHECK_DIST_TO_HOME, 0, e.action.installTtemplate.param4, 300, 0, 0, 0, SMART_ACTION_ALLOW_COMBAT_MOVEMENT, 1, 0, 0, 0, 0, 0, SMART_TARGET_NONE, 0, 0, 0, 0, 1);
                AddEvent(SMART_EVENT_CHECK_DIST_TO_HOME, 0, 0, e.action.installTtemplate.param4>10?e.action.installTtemplate.param4-10:0, 0, 0, 0, SMART_ACTION_ALLOW_COMBAT_MOVEMENT, 0, 0, 0, 0, 0, 0, SMART_TARGET_NONE, 0, 0, 0, 0, 1);
...
{
    mInstallEvents.push_back(CreateEvent(e, event_flags, event_param1, event_param2, event_param3, event_param4, event_param5, action, action_param1, action_param2, action_param3, action_param4, action_param5, action_param6, t, target_param1, target_param2, target_param3, target_param4, phaseMask));
}
...
ObjectList* SmartScript::GetTargets(SmartScriptHolder const& e, Unit* invoker /*= NULL*/)
{
    ...
    switch (e.GetTargetType())
    {
        ...
        {
            if (ObjectList* stored = ResolveTargetList(e.target.stored.id))
            {
                l->assign(stored->begin(), stored->end());
                delete stored;
            }
            break;
        }
...
void SmartScript::AddPendingStoredEvents()
{
    ...
    mStoredEvents.insert(mStoredEvents.end(), mPendingStoredEvents.begin(), mPendingStoredEvents.end());
    mPendingStoredEvents.clear();
}
...
void SmartScript::GetScript()
{
    ...
    if (me)
    {
        if(me->GetDBTableGUIDLow())
            e = sSmartScriptMgr->GetScript(-static_cast<int64>(me->GetDBTableGUIDLow()), mScriptType);
        if (e.empty())
            e = sSmartScriptMgr->GetScript(static_cast<int64>(me->GetEntry()), mScriptType);
        FillScript(e, me, nullptr);
    }
    else if (go)
    {
        e = sSmartScriptMgr->GetScript(-static_cast<int64>(go->GetDBTableGUIDLow()), mScriptType);
        if (e.empty())
            e = sSmartScriptMgr->GetScript(static_cast<int64>(go->GetEntry()), mScriptType);
        FillScript(e, go, nullptr);
    }
    else if (event)
    {
        e = sSmartScriptMgr->GetScript(-static_cast<int64>(event->GetDBTableGUIDLow()), mScriptType);
        if (e.empty())
            e = sSmartScriptMgr->GetScript(static_cast<int64>(event->GetEntry()), mScriptType);
        FillScript(e, event, nullptr);
    }
    else if (trigger)
    {
        e = sSmartScriptMgr->GetScript(static_cast<int64>(trigger->ID), mScriptType);
    ...
    if (obj)//handle object based scripts
    {
        switch (obj->GetTypeId())
        {
            ...
            case TYPEID_EVENTOBJECT:
                ...
                TC_LOG_DEBUG("scripts.ai", "SmartScript::OnInitialize: source is EventObject %u", event->GetEntry());
                break;
            default:
...
void SmartScript::StoreTargetList(ObjectList* targets, uint32 id)
{
    ...
    ObjectGuidVector guids;
    guids.reserve(targets->size());
    for (WorldObject* target : *targets)
        guids.push_back(target->GetGUID());

    delete targets;
    StoreTargetGuids(guids, id);
}

void SmartScript::StoreTargetGuids(ObjectGuidVector const& guids, uint32 id)
{
    (*mTargetStorage)[id] = guids;
}

ObjectGuidVector const* SmartScript::GetStoredTargetGuids(uint32 id) const
{
    auto itr = mTargetStorage->find(id);
    return itr != mTargetStorage->end() ? &itr->second : nullptr;
}
...
ObjectList* SmartScript::ResolveTargetList(uint32 id)
{
    ObjectGuidVector const* guids = GetStoredTargetGuids(id);
    if (!guids)
        return nullptr;
    ...
    for (ObjectGuid const& guid : *guids)
    {
        WorldObject* target = nullptr;
        if (base)
            target = ObjectAccessor::GetWorldObject(*base, guid);
        else if (guid.IsPlayer())
            target = ObjectAccessor::FindPlayer(guid);
        else if (guid.IsGameObject())
            target = ObjectAccessor::FindGameObject(guid);
        else
            target = ObjectAccessor::FindUnit(guid);
        if (target)
...
{
    mEventPhase = mEventPhase > static_cast<uint32>(p) ? mEventPhase - static_cast<uint32>(p) : 0;
}

bool SmartScript::IsInPhase(uint32 p) const
{
    if (!mEventPhase || mEventPhase > 32)
        return false;

    return ((1u << (mEventPhase - 1)) & p) != 0;
}
