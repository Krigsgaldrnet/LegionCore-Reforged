...
void BrawlersGuild::Update(uint32 diff)
{
    for (auto& it : _waitList)
        if (!ObjectAccessor::FindPlayer(m_map, it))
            RemovePlayer(it);

    if (_current && !ObjectAccessor::FindPlayer(m_map, _current))
        EndCombat(false);
...
void BrawlersGuild::RemovePlayers()
{
    ...
    {
        _waitList.remove(it);
        if (auto player = ObjectAccessor::FindPlayer(m_map, it))
    ...
    switch (_brawlState)
    {
        case BRAWL_STATE_WAITING:
            if (!_waitList.empty())
            {
                ...
                if (auto player = ObjectAccessor::FindPlayer(m_map, _current))
                {
                    player->RemoveAura(SPELL_QUEUED_FOR_BRAWL);
